...
const Hero = () => {
  ...
  return (
    <section className="relative bg-gradient-subtle pt-16 pb-24" aria-label="Hero section">
      <div className="container mx-auto px-4">
        ...
        <div className="text-center space-y-8 animate-slide-up">
          ...
          {/* Location and Search */}
          <div className="bg-card rounded-lg p-6 shadow-elegant max-w-2xl mx-auto" role="search">
            <div className="space-y-4">
              <div className="relative">
                <MapPin className="absolute left-3 top-3 w-5 h-5 text-muted-foreground z-10" aria-hidden="true" />
                <Input 
                  ...
                  aria-label="Enter your location or ZIP code"
                ...
                />
                {isSearching && (
                  <Loader2 className="absolute right-3 top-1/2 transform -translate-y-1/2 w-4 h-4 animate-spin text-muted-foreground" aria-label="Searching for locations" />
                )}
          ...
          {/* Stats */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 pt-8 max-w-4xl mx-auto" role="group" aria-label="Platform statistics">
            <div className="text-center">
              <div className="flex items-center justify-center mb-2">
                <Users className="w-6 h-6 text-primary mr-2" aria-hidden="true" />
                <span className="text-3xl font-bold text-foreground" aria-label={`${stats.providers} verified providers`}>{stats.providers}+</span>
              </div>
              <p className="text-lg text-muted-foreground">Verified Providers</p>
            </div>
            <div className="text-center">
              <div className="flex items-center justify-center mb-2">
                <Wrench className="w-6 h-6 text-primary mr-2" aria-hidden="true" />
                <span className="text-3xl font-bold text-foreground" aria-label={`${stats.services} service types`}>{stats.services}+</span>
              </div>
              <p className="text-lg text-muted-foreground">Service Types</p>
            </div>
            <div className="text-center">
              <div className="flex items-center justify-center mb-2">
                <Star className="w-6 h-6 text-accent mr-2" aria-hidden="true" />
                <span className="text-3xl font-bold text-foreground" aria-label={`${stats.rating > 0 ? stats.rating.toFixed(1) : '0.0'} average rating`}>{stats.rating > 0 ? stats.rating.toFixed(1) : '0.0'}</span>
              </div>
